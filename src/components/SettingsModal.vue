...
<template>
  <div class="settings-overlay" @click.self="close">
    <div class="settings-modal">
      <h3>{{ props.entity ? `Настройки сущности: ${pages[0]?.label || props.entity}` : 'Настройки сущностей' }}</h3>
      <div v-if="!props.entity" class="field">
        <label>Pages / Entities</label>
        <div class="tabs">
          <button v-for="(page, pIndex) in pages" :key="pIndex" class="tab" :class="{ active: currentPage === pIndex }"
      ...
      <div class="pages">
        <div v-for="(page, pIndex) in pages" :key="pIndex" class="page" v-show="!props.entity || pIndex === 0">
          ...
          <div class="page-header">
            ...
            <button v-if="!props.entity" class="btn small" @click="removePage(pIndex)">
              Удалить страницу
            </button>
          </div>
          <hr>
          <h4>Поля</h4>
          <div class="fields">
            <div v-for="(f, fIndex) in page.fields" :key="fIndex" class="field-row">
            ...
            <button class="btn small" @click="addField(pIndex)">
              Добавить поле
            </button>
          </div>
        </div>
        <button v-if="!props.entity" class="btn" @click="addPage">Добавить страницу</button>
      </div>
      <div v-if="!props.entity" class="hint">
        Страницы будут сохранены как ключ => массив полей; используйте page key в AdminPage.
      </div>
      <div class="actions">
...
.field-require input:checked+span::before {
  background-color: #333333;
}
