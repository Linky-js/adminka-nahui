...
import BaseRepeater from '@/components/blocks/form/BaseRepeater.vue'
...
const { get, post, listEntityConfigs, getEntityById, createEntity, updateEntity } = useApi()
const { uploadFile } = useFileUpload()

// entity configs from JSON
const entityConfigs = ref({})

async function loadEntityForEdit() {
  if (!entityId.value) {
    // режим создания
    initFormWithData({})
    return
  ...
  try {
    const data = await getEntityById(entity.value, entityId.value)
    ...
    console.error('Ошибка загрузки сущности:', error)
    initFormWithData({})
  }
}
...
const config = computed(() => {
  const val = store.entities ? store.entities[entity.value] : undefined
  if (Array.isArray(val)) return val
  if (val && typeof val === 'object' && Array.isArray(val.fields)) return val.fields
  return entityConfigs.value[entity.value] || []
})

const entityLabel = computed(() => {
  const ent = store.entities ? store.entities[entity.value] : undefined
...
async function loadInitialData() {
  if (props.initialData && typeof props.initialData === 'object' && props.initialData.id && Object.keys(props.initialData).length === 1) {
    ...
    try {
      const fullData = await getEntity('entities/' + entity.value, props.initialData.id);
      if (fullData) {
...
async function buildParamsFromConfig(config, formDataLocal) {
  ...
  for (const field of config.fields) {
    ...
    if (field.type === 'file') {
      ...
      // новый файл
      params[key] = await uploadFile(value.file)
      continue
    }
...
async function saveEntity() {
  const isUpdate = !!entityId.value
  const configCurrent = entityConfigs.value[entity.value] || [];
  ...
  try {
    ...
    if (isUpdate) params.id = entityId.value;

    // --- 2. сохраняем сам объект
    let data;
    console.log('params', params);
    if (isUpdate) {
      data = await updateEntity(entity.value, entityId.value, params)
    } else {
      data = await createEntity(entity.value, params)
    }
    ...
    if (entity.value === 'object' && Array.isArray(formData.srcPhoto)) {
...
<template>
  <div class="content-editor">
    ...
    <component v-for="field in config" :key="field.key" :is="getComponent(field.type)" v-model="formData[field.key]"
      :label="field.label" v-bind="getFieldProps(field)"
      :blocks="field.type === 'repeater' ? field.fields : undefined" />


    <button @click="saveEntity" class="btn primary">Сохранить</button>

    <SettingsModal v-if="showSettings" :entity="entity.value" @close="showSettings = false" />
  </div>
</template>
